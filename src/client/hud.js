...
export class HUD {
  ...
  renderShop() {
    ...
    for (const id of ids) {
      ...
      card.innerHTML = `<div class="it-name">${it.n}</div><div class="it-cost">🪙 ${it.cost}</div>
    ...
    $('shop-hint').textContent = inFountain ? 'At fountain — tap an item to buy.' : 'Shop is available near your fountain only.';
  ...
  initCdMaxes() {
    ...
    for (const slot of ['q', 'e', 'r']) {
      ...
      el.querySelector('span').textContent = ab.name.slice(0, 3).toUpperCase();
      el.title = `${ab.name}: ${ab.desc}`;
      $('mana-' + slot).textContent = ab.mana ? `${ab.mana}mp` : '';
    }
    const sp = SPELLS[me.spell];
    if (sp) {
      this.cdMax['btn-spell'] = sp.cd; $('btn-spell').dataset.max = sp.cd;
      $('spell-label').textContent = sp.name.slice(0, 4).toUpperCase();
  ...
  processEvents(evs) {
    ...
    for (const e of evs) {
      switch (e.type) {
        ...
        case 'gold': if (e.id === this.meId) sfx('gold'); break;
