...
import { cancelRecall, applyBuff, applyCC } from './StatusSystem.js';
...
export function dealDamage(world, ev) {
  ...
  // lane minions only chip heroes (MLBB-style), structures/monsters take full damage
  if (ev.category === 'minion' && isHero(tgt)) dealt *= CONFIG.MINION_VS_HERO;
  ...
  // Frostlens / Winterpact Crown: hero skills chill the target
  if (!ev.noOnHit && isHero(src) && ev.category === 'skill' && src.stats?.chill && isHero(tgt) && actual > 0) {
    applyCC(world, tgt, 'slow', 1.0, { pct: 0.15 });
  }
