...
export class Sim {
  ...
  castSpell(h, aim = {}) {
    ...
    switch (sp) {
      case 'flicker': {
        // default: dash along facing direction (aim falls back to hero's current aim angle)
        const ang = (aim && aim.x !== undefined) ? Math.atan2(aim.y - h.y, aim.x - h.x) : h.aim;
        const tx = h.x + Math.cos(ang) * 360, ty = h.y + Math.sin(ang) * 360;
  ...
  snapshot(viewerTeam) {
    ...
    const packHero = h => ({
      ...
      g: Math.round(h.gold), rg: h.custom.rage ?? undefined, mp: h.maxMana ? Math.round(h.mana) : undefined, it: h.items,
      cds: [+(h.cds.q).toFixed(1), +(h.cds.e).toFixed(1), +(h.cds.r).toFixed(1), +(h.cds.spell).toFixed(1)],
      rc: h.recall ? 1 : 0, ib: h.inBush ? 1 : undefined, dash: h.dashing ? [Math.round(h.dashing.x1), Math.round(h.dashing.y1)] : undefined,
